/*
Zunächst sehen wir uns die ersten 1000 Einträge an, um uns die Daten anzusehen. */
SELECT *
...
Es werden die ride_ids einzeln gezählt. Sollte eine ride_id doppelt vorhanden sein (Zähler größer 1), wird diese ausgegeben. */
SELECT
	ride_id,
	COUNT(ride_id)
FROM 202109_divvy_tripdata
...
CREATE TEMPORARY TABLE column_inverted_check 
SELECT * 
FROM 202109_divvy_tripdata;

-- wir fügen eine Spalte als Platzhalter hinzu
ALTER TABLE column_inverted_check
...
Wir vergleichen die Werte der neuen Tabelle mit denen der alten Tabelle für die Stellen, bei denen die Fahrzeit in der Original-Tabelle negativ ist.
...
SELECT 
	...
	column_inverted_check.started_at AS changed_start,
	column_inverted_check.ended_at AS changed_end,
	TIMESTAMPDIFF(SECOND, 202109_divvy_tripdata.started_at, 202109_divvy_tripdata.ended_at) AS original_ride_duration,
	TIMESTAMPDIFF(SECOND, column_inverted_check.started_at, column_inverted_check.ended_at) AS changed_ride_duration
FROM 
	202109_divvy_tripdata
INNER JOIN 
	column_inverted_check
ON 202109_divvy_tripdata.ride_id = column_inverted_check.ride_id
WHERE TIMESTAMPDIFF(SECOND, 202109_divvy_tripdata.started_at, 202109_divvy_tripdata.ended_at) < 0
ORDER BY TIMESTAMPDIFF(SECOND, column_inverted_check.started_at, column_inverted_check.ended_at) DESC;
-- Wir sehen, dass die Änderungen korrekt durchgeführt wurden.
...
Da wir alle folgenden Abfragen nur auf für uns relevante Spalten beziehen wollen, erzeugen wir eine temporäre Tabelle mit den für uns wichtigen Spalten, um die Abfragen übersichtlicher zu halten.
Alle Fahrzeiten werden auf 2 Nachkommastellen gerundet. */
...
-- Mietdauer nach Nutzertyp unter der Woche 
SELECT
	member_casual,
...
-- Anzahl der Casuals und Members, die am Wochenende mieten 
SELECT
	member_casual AS member_type,
	COUNT(member_casual) AS quantity
FROM RideDuration
...
-- Anzahl der Casuals und Members, die unter der Woche mieten 
SELECT
	member_casual AS member_type,
	COUNT(member_casual) AS quantity
FROM RideDuration
